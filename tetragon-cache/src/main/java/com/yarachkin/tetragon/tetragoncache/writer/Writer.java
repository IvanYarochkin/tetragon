...
public class Writer {
    ...
    public void write() throws CacheTetragonException {
        Reader.getInstance().createFileIfNotExists(Reader.getInstance().acquireFilePath());
        try (FileWriter fileWriter = new FileWriter(Reader.getInstance().acquireFilePath());
